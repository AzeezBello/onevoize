...
export async function POST(request: Request) {
  try {
    ...
    const payment = await paystackInitialize({ email, amount, reference, callbackUrl: `${base}/donate/complete?gateway=paystack&reference=${encodeURIComponent(reference)}`, metadata: { donor_name: anonymous ? undefined : name, donation_type: body.donationType || 'one_time' } })
