...
export default function Home() {
  return <>
    ...
    <section className="section home-leadership">
      <div className="container"><div className="home-section-heading"><div><div className="eyebrow">Leadership</div><h2 className="serif">People entrusted to serve.</h2></div><Link href="/about#exco-members" className="text-link">Meet the Exco <ArrowRight size={16}/></Link></div><div className="home-trustee-grid">{featuredTrustees.map((person) => <Link href="/exco" className="home-trustee" key={person.name}><div className="home-trustee-image"><Image src={person.image} alt={portraitAlt(person)} fill sizes="(max-width: 640px) 100vw, (max-width: 800px) 50vw, 33vw"/></div><strong>{person.name}</strong><span>{person.role}</span></Link>)}</div></div>
    </section>
