...
export default function Home() {
  return <>
    ...
    <section className="section home-leadership">
      <div className="container"><div className="home-section-heading"><div><div className="eyebrow">Leadership</div><h2 className="serif">People entrusted to serve.</h2></div><Link href="/about#exco-members" className="text-link">Meet the Exco <ArrowRight size={16}/></Link></div><div className="home-trustee-grid">{trustees.map(([name, role, image]) => <Link href="/about#exco-members" className="home-trustee" key={name}><div className="home-trustee-image"><Image src={image} alt={`${name}, ${role}`} fill sizes="(max-width: 640px) 100vw, (max-width: 800px) 50vw, 33vw"/></div><strong>{name}</strong><span>{role}</span></Link>)}</div></div>
    </section>
