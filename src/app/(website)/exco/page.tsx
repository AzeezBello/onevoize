...
const card = { background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 22, overflow: 'hidden' } as const
...
export default function Exco() {
  return <>
    ...
    <section className="section"><div className="container"><div className="eyebrow">Executive Committee</div><h2 className="serif" style={{ fontSize: 'clamp(34px,5vw,52px)', margin: '10px 0 30px' }}>Leadership with purpose and accountability.</h2><div className="grid-auto">{members.map(([name, role, image]) => <article key={name} style={card}><div className="portrait" style={{ height: 300, borderRadius: 0 }}><Image src={image} alt={`${name}, ${role}`} fill sizes={sizes}/></div><div style={{ padding: 22 }}><h3 className="serif" style={{ fontSize: 26, margin: 0 }}>{name}</h3><div className="eyebrow" style={{ marginTop: 8 }}>{role}</div></div></article>)}</div></div></section>
    <section className="section" style={{ background: 'var(--surface)' }}><div className="container"><div className="eyebrow">Club Members</div><h2 className="serif" style={{ fontSize: 'clamp(34px,5vw,52px)', margin: '10px 0 30px' }}>The people who make the club.</h2><div className="grid-auto">{clubMembers.map(([name, image]) => <article key={name} style={{ ...card, borderRadius: 20 }}><div className="portrait" style={{ height: 280, borderRadius: 0 }}><Image src={image} alt={`${name}, Global Voize Club member`} fill sizes={sizes}/></div><div style={{ padding: 18 }}><h3 className="serif" style={{ fontSize: 23, margin: 0 }}>{name}</h3><div className="eyebrow" style={{ marginTop: 7 }}>Member</div></div></article>)}</div></div></section>
  </>
}
